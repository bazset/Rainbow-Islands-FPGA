...
module jttc0030cmd (
    ...
    // jtframe_dual_ram replaced with a plain inferred dual-port RAM,
    // kept SYNCHRONOUS/REGISTERED-READ (1-cycle latency) to match what
    // this file's own README explicitly relies on: "The BRAM's 1-cycle
    // read latency hides inside the slower cen-gated MCU cycle, so the
    // internal read mux is combinational." A first version of this
    // substitution used a combinational (0-cycle) read instead, which
    // would have changed that timing relationship -- fixed before it
    // ever shipped, not found by testing (no ROM to test against yet).
    // (* ramstyle = "no_rw_check" *) is the standard Quartus attribute
    // for "I don't rely on mixed-port read-during-write behaviour" --
    // without it Quartus reported this as "uninferred due to unsupported
    // read-during-write behavior" and fell back to ~65,000 flip-flops
    // instead of one M10K block. The attribute is safe here because the
    // two sides address independently-banked 1KB windows (bank_mcu vs
    // bank_68k) and the original jtframe_dual_ram this replaced makes
    // the same no-mixed-port-RDW assumption.
    (* ramstyle = "no_rw_check" *) reg [7:0] sram_arr [0:8191];
    reg [7:0] sram_qmcu_r, sram_q68_r;
    always @(posedge clk) begin
        if (mcu_wr & mcu_sram) sram_arr[{bank_mcu, mcu_addr[9:0]}] <= mcu_dout;
        sram_qmcu_r <= sram_arr[{bank_mcu, mcu_addr[9:0]}];
    end
    always @(posedge clk) begin
        if (k68_wr & k68_sram) sram_arr[{bank_68k, addr[9:0]}] <= din;
        sram_q68_r <= sram_arr[{bank_68k, addr[9:0]}];
    end
    assign sram_qmcu = sram_qmcu_r;
    assign sram_q68  = sram_q68_r;

    (* ramstyle = "no_rw_check" *) reg [7:0] iram_arr [0:255];
    reg [7:0] iram_q_r;
    always @(posedge clk) begin
        if (mcu_wr & mcu_iram) iram_arr[mcu_addr[7:0]] <= mcu_dout;
        iram_q_r <= iram_arr[mcu_addr[7:0]];
    end
    assign iram_q = iram_q_r;
